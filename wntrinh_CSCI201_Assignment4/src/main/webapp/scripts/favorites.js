...
function getRestaurants() {
	...
	xhr.open("post", "/wntrinh_CSCI201_Assignment4/getFavorites?", true);
    xhr.onreadystatechange = function() {
		if (xhr.readyState == XMLHttpRequest.DONE) {
			...
			if (status == 0 || (status >= 200 && status < 400)) {
				...
				jsonObject.sort((a, b) => b.fav_id - a.fav_id);
                ...
                for (var i = 0; i < size; ++i) {
				    ...
				    urlLink.textContent = restaurant.url;
				    ...
				    imageElement.addEventListener("click", function() {
				        ...
				        function addToFavoritesHandler(event) {
				            addToFavorites(event, restaurant.restaurant_id, favoriteButton, removeFavButton);
				        }
				        
				        function removeFromFavoritesHandler(event) {
				            removeFromFavorites(event, restaurant.restaurant_id, favoriteButton, removeFavButton);
				        }
					    ...
					    submitButton.addEventListener("click", function(event) {
							if (dateInput.value != "" && timeInput.value != "") {
							    addToReservationsHandler(event, restaurant.restaurant_id, dateInput.value, timeInput.value, notesInput.value);
							    dateInput.value = "";
							    timeInput.value = "";
							    notesInput.value = "";
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == XMLHttpRequest.DONE) {
            ...
            if (status == 0 || (status >= 200 && status < 400)) {
                ...
                t.innerHTML = xhr.responseText + "\'s Favorites:";
            } else {
                console.log("Error: " + status + " code.");
            }
        }
        ...
        if (xhr.readyState == XMLHttpRequest.DONE) {
            ...
            if (status == 0 || (status >= 200 && status < 400)) {
                ...
                if (response == "Send restaurant data back here.") {
                    ...
                    callback("Unexpected response", 2);
                }
            } else {
                console.log("Error: " + status + " code.");
                callback("Request failed", 2);
            }
        }
    };
			...
			if (status == 0 || (status >= 200 && status < 400)) {
                ...
                for (var i = 0; i < size; ++i) {
				    ...
				    imageElement.addEventListener("click", function() {
						...
						resultsContainer.appendChild(showDetailsFav(restaurant));
...
function sort(method, jsonArr) {
    switch(method) {
        ...
        case 'ZtoA':
            ...
            break;
        case 'HighestRating':
            jsonArr.sort((a, b) => b.rating - a.rating);
            break;
        case 'LowestRating':
            jsonArr.sort((a, b) => b.rating - a.rating);
            jsonArr.reverse();
            break;
        case 'MostRecent':
            jsonArr.sort((a, b) => b.fav_id - a.fav_id);
            break;
        case 'LeastRecent':
            jsonArr.sort((a, b) => a.fav_id - b.fav_id);
            break;
        default:
            jsonArr.sort((a, b) => b.fav_id - a.fav_id);
            break;
        ...
        imageElement.addEventListener("click", function() {
            ...
            if (userEmail) {   
			    ...
			    reserveButton.addEventListener("click", function(event) {
			        event.preventDefault();
			        clicked++;
			        reservationFormDiv.style.display = (clicked % 2 === 0) ? "none" : "block";
			    });
			    ...
			    imageElement.addEventListener("click", function() {
			        ...
			        resultsContainer.appendChild(showDetails(restaurant));
			        // Update display property as needed when the details are shown
			        favoriteButton.style.display = "block";
			        removeFavButton.style.display = "block";
			        reserveButton.style.display = "block";
			        reservationFormDiv.style.display = "none";
